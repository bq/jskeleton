describe('renderer', function() {
  ...
  describe('when renderer is called with arguments', function(){
    beforeEach(function(){
        this.template = '<span>foo</span>';
        this.renderSpy = this.sinon.spy(Marionette.Renderer, 'render');
        this.result = Marionette.Renderer.render(this.template, this.data);
    });

    it('should will called with data and template arguments', function(){
        expect(this.renderSpy).to.have.been.calledOnce.and.calledWith(this.template,this.data);
    });
  ...
  /*describe('when template is a function', function(){
    beforeEach(function(){
        ...
        this.fnTemplate = _.template('<span>foo</span>');
        this.result = Marionette.Renderer.render(this.fnTemplate, this.data);
    });
  ...
  /*describe('when given a template and data to render', function() {
    beforeEach(function() {
      ...
      this.result = Marionette.Renderer.render(this.templateSelector, this.data);
    });
